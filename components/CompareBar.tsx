      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
        ...
        <div className="flex flex-col gap-2 sm:hidden">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex-1 flex items-center gap-2 min-w-0 overflow-hidden">
              {selected.map((l) => l && (
                <div key={l.id} className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 min-w-0 flex-1">
                  <span className="text-xs text-slate-800 font-medium truncate">{l.title}</span>
                  <button onClick={() => remove(l.id)} className="shrink-0 text-slate-400 hover:text-red-500">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
              {ids.length < 2 && (
                <div className="flex-1 flex items-center justify-center border-2 border-dashed border-slate-200 rounded-lg px-2 py-1 text-xs text-slate-400">
                  + Add one more
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex-1 py-2 bg-teal-600 text-white text-sm font-semibold rounded-lg hover:bg-teal-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Compare now
            </button>
          </div>
        ...
        <div className="hidden sm:flex items-center gap-4">
          ...
          <div className="flex-1 flex items-center gap-3 min-w-0">
            {selected.map((l) => l && (
              <div key={l.id} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 min-w-0">
                ...
                <button onClick={() => remove(l.id)} className="shrink-0 text-slate-400 hover:text-red-500 transition-colors" aria-label="Remove from compare">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={clear} className="text-xs text-slate-500 hover:text-slate-700 transition-colors">Clear</button>
            <button
              disabled={ids.length < 2}
              onClick={() => router.push(`/compare?ids=${ids.join(',')}`)}
